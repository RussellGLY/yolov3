...
class LoadSequenceDataset(Dataset):
    ...
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        images_path = [line.split(',')[0].replace("'",'') for line in filepaths] 
        annos_path  = [line.split(',')[1].replace("'",'') for line in filepaths]        
    
        self.images_path = images_path
        self.annos_path  = annos_path
        
        n = len(images_path)                               # number of video clips in list 
        assert n > 0, 'No videos found in %s. See %s' % (path, help_url)
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)  # batch index
        nb = bi[-1] + 1  # number of batches
        #print("number of batches: ",nb,bi)
        ...
        print("Number of videos:",len(self.images_path),"Number of annos:",len(self.annos_path))
        
        self.imgs = []
        self.imgs_hw0  = []
        self.imgs_hw  = []
        self.shapes_all = []
        pbar = tqdm(range(len(images_path)), desc='Caching images ')
        for _,i in enumerate(pbar):
            if i == 10: break # for testing remove during training
            imgs     = []
            imgs_hw0 = []
            imgs_hw  = []
            shapes_seq  = []
            for img_path in os.listdir(images_path[i]):
                img, img_hw0, img_hw = load_image_new(self, images_path[i]+img_path)
                img, ratio, pad = letterbox(img, auto=False, scaleup=self.augment)
                shapes = [img_hw0, ((img_hw[0] / img_hw0[0], img_hw[1] / img_hw[1]), pad)]
                imgs.append(img)
                imgs_hw0.append(img_hw0)
                imgs_hw.append(img_hw)
                shapes_seq.append(shapes)
            self.shapes_all.append(shapes_seq)
            self.imgs.append(imgs)
            self.imgs_hw0.append(imgs_hw0)
            self.imgs_hw.append(imgs_hw)        
        self.labels = []
        (h,w) = img_hw
        pbar = tqdm(range(len(annos_path)), desc='Caching labels')
        for _,i in enumerate(pbar):
            if i == 10: break   # for testing remove during training
            with open(annos_path[i].replace("\n",''), 'r') as f:
                    x = np.array([x.split()[0:5] for x in f.read().splitlines()], dtype=np.float32)
    ...
    def __getitem__(self, index):
        return self.imgs[index],self.labels[index],self.shapes_all[index]
    
    @staticmethod
    def collate_fun(data):
        video_images,video_labels,shapes = zip(*data)
        sequence_length = 16        #hyperparameter
        len_seq = [len(image)//sequence_length for image in video_images]

        full_mini_images = []
        full_mini_labels = []

        for ind in range(len(video_labels)):    # for each video sequence in batch 
            images_mini_batch=[]
            annos_mini_batch =[]
            for i in range(len_seq[ind]):   # get sequences of sequence length with annos and orig shapes and aspect ratios
                images_mini_batch.append(video_images[ind][i*sequence_length:(i+1)*sequence_length])
                annos_mini_batch.append(video_labels[ind][i*sequence_length:(i+1)*sequence_length])
            images_mini_batch.append(video_images[ind][len(video_images[ind])-sequence_length:len(video_images[ind])])
            annos_mini_batch.append(video_labels[ind][i*sequence_length:(i+1)*sequence_length])
            full_mini_images.append(images_mini_batch)
            full_mini_labels.append(annos_mini_batch)
        return full_mini_images,full_mini_labels,shapes


def load_image_new(self, img_path):
